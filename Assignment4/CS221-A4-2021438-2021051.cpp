    ...
    for (int row = 0; row < R; row++)
        for (int col = 0; col < C; col++)
        {
            ...
            if (search2D(grid, row, col, word, R, C))
                oFile << word << " (" << row + 1 << "," << col + 1 << ") (" << row + 1 << "," << word.length() + col << ")" << endl;
...
int main()
{
    ...
    {
        if (data.size() > 2)
            patternSearch((char *)grid, data, R, C,oFile);
    }
    return 0;
}
